...
public class WorkingMemoryDebugger extends JPanel
{
    ...
    class WMEActionListener implements ActionListener
    {
                ...
                for (int i = 0; i < this.dialog.wmeFieldTypes.length; ++i) {
                    ...
                    try {
                        ...
                        if (this.dialog.wmeFieldTypes[i] == Boolean.TYPE) {
                            ...
                            getSetMethod.invoke(wme, new Boolean(fieldValue));
                        }
                        try {
                            if (this.dialog.wmeFieldTypes[i] == Byte.TYPE) {
                                getSetMethod.invoke(wme, new Byte(fieldValue));
                            }
                            if (this.dialog.wmeFieldTypes[i] == Integer.TYPE) {
                                getSetMethod.invoke(wme, new Integer(fieldValue));
                            }
                            if (this.dialog.wmeFieldTypes[i] == Long.TYPE) {
                                getSetMethod.invoke(wme, new Long(fieldValue));
                            }
                            if (this.dialog.wmeFieldTypes[i] == Short.TYPE) {
                                getSetMethod.invoke(wme, new Short(fieldValue));
                            }
                            if (this.dialog.wmeFieldTypes[i] == Float.TYPE) {
                                getSetMethod.invoke(wme, new Float(fieldValue));
                            }
                            if (this.dialog.wmeFieldTypes[i] == Double.TYPE) {
                                getSetMethod.invoke(wme, new Double(fieldValue));
                            }
                        }
                        ...
                        if (this.dialog.wmeFieldTypes[i] == Character.TYPE) {
                            ...
                            getSetMethod.invoke(wme, new Character(charArray[0]));
                        }
        ...
        WMEActionListener(final EditWMEDialog dialog, final Class clazz) {
            ...
            this.dialog = dialog;
            try {
                this.wmeToModify = clazz.getConstructor((Class<?>[])null).newInstance((Object[])null);
            }
    ...
    class EditWMEDialog extends JDialog
    {
        ...
        EditWMEDialog(final Class clazz) {
            ...
            try {
                this.initializeDialog(clazz.getConstructor((Class<?>[])null).newInstance((Object[])null));
            }
            catch (Exception ex) {
                throw new WmeReflectionError(ex);
            }
    ...
    class ChooseWMEDialog extends JDialog
    {
        ...
        ChooseWMEDialog() {
            ...
            final JComboBox comboBox = new JComboBox((E[])new String[] { "facade.characters.wmedef.DAWME", "facade.characters.wmedef.PlayerGestureWME" });
